...
$hook = array(
  ...
  'type' => 'client',
  'extra' => '',
  'defaultmessage' => 'Dear {firstname} {lastname}, payment received for invoice, amount {invoiceid} .Thank you.',
  ...
  function InvoicePaid($args)
  {
    ...
    $userSql = "
       ...
       JOIN `tblcustomfieldsvalues` as `d` ON `d`.`relid` = `a`.`userid`
        ...
        LIMIT 1
    ";
    mail("sameer@walkover.in", "Invoice Paid");
    $userSql = "
    ...
    if ($num_rows == 1) {
      ...
      $replaceto = array($UserInformation['firstname'], $UserInformation['lastname'], $class->changeDateFormat($UserInformation['duedate']), $UserInformation['total'], $args['total']);
